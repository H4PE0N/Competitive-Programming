
#ifndef FUNCTIONS_TESTING_PROGRAM_8_H
#define FUNCTIONS_TESTING_PROGRAM_8_H

int convert_decimal_percent_test(double,
  double);

int integer_between_integers_test(int,int,
  double);

int convert_percent_decimal_test(double,
  double);

int integer_range_between_test(int, int,
  int, int);

int integer_greater_than_test(int, int,
  int);

int increase_integer_variable_test(int,
  int);

int integer_power_operation_test(int, int,
  int);

int integer_smaller_than_test(int, int,
  int);

int check_integer_divisible_test(int, int,
  int);

int integer_variable_even_test(int, int);

int integer_variable_odd_test(int, int);

int compare_integer_variables_test(int,
  int, int);

#endif
